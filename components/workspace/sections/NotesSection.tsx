...
export default function Notes({ section, onUpdate }: Props) {
    ...
    return (
        <div className="max-w-6xl mx-auto h-[max(calc(100vh-160px),600px)] flex flex-col lg:flex-row gap-6 animate-fade-in relative z-10">
            ...
            {/* Sidebar List */}
            <div className="w-full lg:w-80 shrink-0 flex flex-col min-h-0 surface rounded-2xl border border-white/5 shadow-xl animate-slide-up bg-obsidian-surface/60 backdrop-blur-sm">
                <div className="p-5 border-b border-border/50 flex items-center justify-between z-10 sticky top-0 bg-obsidian/40 backdrop-blur-xl rounded-t-2xl">
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-2">
                    ...
                    {section.data.length === 0 && (
                        <div className="p-6 text-center text-text-muted text-sm border border-dashed border-white/10 rounded-xl m-2">
            ...
            {/* Editor Main */}
            {activeNote ? (
                <div className="flex-1 flex flex-col min-h-0 surface rounded-2xl border border-white/5 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.5)] bg-obsidian animate-slide-up shadow-indigo-500/5 relative overflow-hidden" style={{ animationDelay: '0.1s' }}>

                    {/* Editor glow effect inner */}
                    <div className="absolute top-0 right-0 w-64 h-64 bg-yellow-500/5 blur-[100px] pointer-events-none rounded-full" />

                    <div className="p-6 sm:p-8 lg:px-12 lg:pt-10 flex-shrink-0 border-b border-white/5 pb-6 bg-obsidian-surface/30">
                        <input
                            type="text"
                            value={activeNote.title}
                            onChange={(e) => updateNote(activeNote.id, { title: e.target.value })}
                            className="w-full bg-transparent border-none text-2xl sm:text-3xl font-display font-black text-text-primary placeholder:text-text-muted/40 focus:outline-none mb-2"
                            placeholder="Note Title"
                        />
                        <div className="flex items-center gap-4 text-xs font-bold text-text-muted/60 uppercase tracking-widest">
                    ...
                    <div className="flex-1 overflow-y-auto lg:px-12 px-6 sm:px-8 py-8 relative group">
                        ...
                        <textarea
                            className="w-full h-full bg-transparent border-none resize-none text-[15px] sm:text-base leading-8 font-serif text-text-primary placeholder:text-text-muted focus:outline-none focus:ring-0 custom-scrollbar relative z-10 drop-shadow-sm"
                ...
                <div className="flex-1 flex items-center justify-center surface rounded-2xl border-white/5 border-dashed border shadow-lg m-2 lg:m-0">
                    <p className="text-text-muted font-display text-lg">Select or create a note to begin.</p>
                </div>
            )}
        </div>
    );
