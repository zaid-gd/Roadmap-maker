                    ...
                    {section.data.map((milestone, i) => {
                        ...
                        const stateColor = milestone.completed
                            ...
                            : inProgress
                                ...
                                : "border-white/5 bg-obsidian-surface/50 opacity-80 hover:opacity-100";
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className={`rounded-2xl overflow-hidden transition-all duration-500 border-l-[3px] border-r border-t border-b hover:-translate-y-1 backdrop-blur-sm ${stateColor} ${isExpanded && !inProgress && !milestone.completed ? "ring-1 ring-white/10" : ""}`}>
                                    ...
                                    >
                                        <div
                                            className={`relative flex items-center justify-center w-6 h-6 rounded flex-shrink-0 transition-all duration-300 ${milestone.completed ? 'bg-emerald-500' : 'bg-obsidian-elevated border border-white/10 group-hover:border-indigo-400'
                                                }`}
                                            onClick={(e) => {
                                                e.stopPropagation();
                                        ...
                                        >
                                            {milestone.completed && <span className="text-white text-xs font-bold shadow-sm">✓</span>}
                                        </div>

                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2">
                                                <h3 className={`font-display font-bold text-[17px] transition-colors duration-300 ${milestone.completed ? "text-text-muted" : "text-text-primary"
                                                    } ${inProgress && !milestone.completed ? 'text-indigo-50' : ''}`}>
                                                    {milestone.title}
                                                </h3>
                                                {inProgress && <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide bg-indigo-500/20 text-indigo-300 animate-pulse border border-indigo-500/30">Active</span>}
                                            </div>
                                            <p className={`text-sm mt-1 line-clamp-1 transition-colors ${milestone.completed ? "text-text-muted/50" : "text-text-secondary"}`}>
                                        ...
                                        <div className={`flex items-center justify-center w-8 h-8 rounded-full bg-white/5 transition-transform duration-300 group-hover:bg-white/10 ${isExpanded ? "rotate-180 bg-white/10" : ""}`}>
                                            <span className="text-xs text-text-secondary group-hover:text-text-primary">▼</span>
                                        </div>
                                    </button>
                                    ...
                                    <div className={`grid transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] ${isExpanded ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
                                        <div className="overflow-hidden">
                                            <div className="px-5 sm:px-6 pb-6 pt-2 space-y-8 border-t border-white/5 mx-2 bg-gradient-to-b from-transparent to-obsidian/30">

                                                <p className="text-text-secondary text-sm leading-relaxed whitespace-pre-line bg-obsidian-surface/60 p-4 rounded-xl border border-white/5 shadow-inner">
                                                    {milestone.description}
                                                </p>
                                                        ...
                                                        <div className="space-y-2 grid grid-cols-1">
                                                            {milestone.tasks.map((task) => (
                                                                <label key={task.id} className={`group relative flex items-start gap-3 p-3 rounded-xl hover:bg-obsidian-hover border border-white/5 cursor-pointer transition-all duration-300 ${task.completed ? "bg-emerald-500/5 border-emerald-500/10" : "bg-obsidian-elevated/40"}`}>
                                                                    ...
                                                                    />
                                                                    <span className={`text-sm leading-relaxed transition-all duration-300 ${task.completed ? "text-text-muted line-through decoration-white/20" : "text-text-primary group-hover:text-indigo-100"}`}>
                                                                        {task.title}
                                                                    </span>
                                                                </label>
                                                            ))}
                                                        ...
                                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                            {milestone.resources.map((res) => (
                                                                <button
                                                                    key={res.id}
                                                                    type="button"
                                                                    className="flex items-start gap-3 p-3 rounded-xl bg-obsidian-elevated/40 hover:bg-obsidian-hover border border-white/5 text-left transition-all duration-200 group hover:-translate-y-0.5 hover:shadow-lg"
                                                                    onClick={() => setViewerUrl(res.url)}
                                                                >
                                                                    <div className="w-8 h-8 rounded-lg bg-amber-500/10 flex items-center justify-center text-sm shrink-0 border border-amber-500/20 group-hover:scale-110 group-hover:bg-amber-500/20 transition-all">
                                                                        <span className="drop-shadow-[0_0_8px_rgba(251,191,36,0.4)]">📄</span>
                                                                    </div>
                                                                    <div className="min-w-0 flex-1">
                                                                        <span className="text-sm font-medium text-amber-200 group-hover:text-amber-100 block truncate transition-colors">{res.title}</span>
                                                                        {res.description && <span className="text-xs text-text-muted block truncate mt-0.5">{res.description}</span>}
                                                                    </div>
                                                                </button>
                                                            ))}
                                                        </div>
