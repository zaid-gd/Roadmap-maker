...
export default function SmartEmbed({ url, title, description }: SmartEmbedProps) {
    ...
    if (iframeBlocked) {
        return (
            <div className="w-full border border-white/10 bg-obsidian-elevated/60 p-6 flex items-center gap-5 transition-all hover:border-indigo-500/30 hover:bg-obsidian-elevated/80 group">
                <img
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-text-muted/50 text-[10px] uppercase tracking-widest mt-1">{getDomain(url)}</p>
                </div>
                <a
                    href={url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="shrink-0 font-sans-display text-[10px] uppercase tracking-widest text-indigo-400 hover:text-white border border-indigo-500/30 hover:border-indigo-500 px-4 py-2.5 transition-all bg-indigo-500/5 hover:bg-indigo-500 whitespace-nowrap"
                >
                    Open Resource →
                </a>
            </div>
    ...
    return (
        <div className="w-full relative">
            {loading && (
                <div className="absolute inset-0 flex items-center justify-center bg-obsidian-elevated/80 z-10 border border-white/5">
                    <div className="flex flex-col items-center gap-3">
                        <div className="w-6 h-6 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin" />
                        <span className="text-text-muted text-xs">Loading resource...</span>
                    </div>
                </div>
            )}
            <iframe
                ...
                src={url}
                title={title}
                className="w-full h-[500px] border border-white/5 rounded-lg"
