...
export default function WorkspacesPage() {
    ...
    useEffect(() => {
        setWorkspaces(storage.getRoadmaps());
        setMounted(true);
        if (storage.syncFromCloud) {
            void storage.syncFromCloud().then((synced) => setWorkspaces(synced));
        }
    }, [storage]);
            ...
            <main className="flex-1 w-full max-w-6xl mx-auto px-6 py-24">
                ...
                {workspaces.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-32 text-center bg-obsidian-surface/30 rounded-2xl border border-border border-dashed">
                        ...
                        <h2 className="text-xl font-display font-semibold text-white mb-8 max-w-sm">
                            No workspaces yet. Generate your first roadmap!
                        </h2>
                        <Link
                            href="/"
